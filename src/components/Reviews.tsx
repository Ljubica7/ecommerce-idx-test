...
function ReviewColumn({
    ...
    reviews: string[]
    className?: string
    reviewClassName?: (reviewIndex: number) => string; msPerPixel?: number
}) {
        ...
        {isInView ?
            <>
                <ReviewColumn reviews={[...column1, ...column3.flat(), ...column2]} reviewClassName={(reviewIndex) => cn({
                    "md:hidden": reviewIndex >= column1.length + column3[0].length, "lg:hidden": reviewIndex >= column1.length
                })} msPerPixel={10} />

                <ReviewColumn reviews={[...column2, ...column3[1]]} 
                className='hidden md:block' reviewClassName={(reviewIndex) => reviewIndex>= column2.length ? 'lg:hidden': ''} msPerPixel={15} />

                <ReviewColumn reviews={column3.flat()} 
                className='hidden md:block'  msPerPixel={10} />
            </>
            : null
        }
...
export function Reviews() {
    return <MaxWidthWrapper className="relative max-w-5xl">
        <img src="/what-people-are-buying.png" aria-hidden="true" className="absolute select-none hidden xl:block -left-32 top-1/3" />
